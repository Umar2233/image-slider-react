...
const Slider = () => {
    ...
    const handleMove = (d) => {
        ...
        if(d === 'l') {
            slideNumber === 0 ? setSlideNumber(5) : setSlideNumber(slideNumber - 1)
        }
        else{
            slideNumber === 5 ? setSlideNumber(0) : setSlideNumber(slideNumber + 1)
        }
    }
                    ...
                    <div className="images">
                        {photos.map((photo, i) => (
                            <div className="singleImage" key={i} >
                                <img src={photo.src} onClick={() => handleOpen(i)} alt='' className="image" />
                            </div>
                        ))}
                    </div>
                </div>
